...
import { mockEnv, parseJson, STANDARD_ENV } from '../_test-utils/helpers.ts'
...
function buildWebhookRequest(body: string, signatureHeader?: string): Request {
...
Deno.test('verifyWebhookSignature', async (t) => {
  await t.step('returns true for valid signature', async () => {
    ...
    const result = await verifyWebhookSignature(body, `time=${ts},sig1=${sig}`, secret)
    assertEquals(result, true)
  })

  await t.step('returns false for wrong signature', async () => {
    const result = await verifyWebhookSignature('{"uid":"abc"}', 'time=123,sig1=deadbeef', 'secret')
  ...
  await t.step('returns false when header has no sig1', async () => {
    const result = await verifyWebhookSignature('body', 'time=123', 'secret')
    assertEquals(result, false)
  })
})
...
Deno.test('chat-media-webhook handler', async (t) => {
  ...
  await t.step('returns 500 when webhook secret not found in vault', async () => {
    ...
    const res = await handler(buildWebhookRequest('{"uid":"abc"}'))
  ...
  await t.step('returns 403 for invalid signature', async () => {
    setup()
    mockVaultSecrets()
    const res = await handler(buildWebhookRequest('{"uid":"abc"}', 'time=123,sig1=bad'))
  ...
  await t.step('returns 204 for event without uid', async () => {
    ...
    const res = await handler(buildWebhookRequest(body, `time=${ts},sig1=${sig}`))
  ...
  await t.step('updates attachment to ready on readyToStream', async () => {
    ...
    const updatePayload = mockState.lastUpdatePayload.get('media_attachments') as Record<
      string,
      unknown
    >
