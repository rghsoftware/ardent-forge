...
1. **As a user**, I can tap the COMMS tab in the bottom nav (mobile) or sidebar (large screens) to see my conversations.
...
| --- | ------------------------------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------- |
| M1  | COMMS tab in mobile bottom nav (`chat` icon) and large screen sidebar (below VAULT)                     | --        | Tab visible, routes to `/comms`                                 |
...
| --- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| S1  | Attachment picker bottom sheet (mobile) / dropdown (large screens) with video, photo, workout, file options | Picker opens with 4 options         |
...
| ----- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| TA-1  | COMMS tab appears in both mobile bottom nav and large screen sidebar                                  | Visual inspection at both breakpoints                |
...
| OQ-4 | Mobile bottom nav currently has 5 tabs + avatar. Adding COMMS makes 6 + avatar. Is that too many?                                                | Navigation UX                 | **Propose:** Replace "Social" (`/groups`) with "Comms" (`/comms`) on mobile. Groups remain accessible from the large screen sidebar and via deep links. Or: keep 5 tabs, replace Social with Comms, move Groups under a secondary nav. Need user decision. |
