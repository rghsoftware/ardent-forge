            ...
            if (block.type === 'tool_use') {
              ...
              if (name === 'Bash' && input?.command) {
                ...
                if (cmd.includes('build') || cmd.includes('test'))
                  summary.buildTestResults.push({ command: input.command, result: 'executed' })
              }
            }
          }
        }
...
export function formatMarkdown(
  ...
  if (summary.userRequests.length) {
    ...
    summary.userRequests.forEach((r) => lines.push(`- ${r}`))
    lines.push('')
  }
  ...
  if (summary.tasksCreated.length) {
    ...
    summary.tasksCreated.forEach((t) => lines.push(`- **${t.subject}**: ${t.description}`))
    lines.push('')
  }
  ...
  if (summary.subAgentCalls.length) {
    ...
    summary.subAgentCalls.forEach((c) => lines.push(`- **${c.agent}**: ${c.description}`))
    lines.push('')
  }
  if (summary.skillsLoaded.length) {
    lines.push('## Skills Loaded')
    summary.skillsLoaded.forEach((s) => lines.push(`- ${s}`))
    lines.push('')
  }
...
export function saveBackup(
  ...
  try {
    ...
    const existing = existsSync(backupDir)
      ? readdirSync(backupDir).filter((f) => f.endsWith('.md') && /^\d+-/.test(f))
      : []
