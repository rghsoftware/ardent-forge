  ...
  if (matchedSkills.length > 0 || matchedAgents.length > 0) {
    ...
    if (matchedSkills.length > 0) {
      ...
      if (g.critical.length) {
        ...
        g.critical.forEach((s) => lines.push('  -> ' + s.name))
        lines.push('')
      }
      if (g.high.length) {
        lines.push('RECOMMENDED SKILLS:')
        g.high.forEach((s) => lines.push('  -> ' + s.name))
        lines.push('')
      }
      if (g.medium.length) {
        lines.push('SUGGESTED SKILLS:')
        g.medium.forEach((s) => lines.push('  -> ' + s.name))
        lines.push('')
      }
      if (g.low.length) {
        lines.push('OPTIONAL SKILLS:')
        g.low.forEach((s) => lines.push('  -> ' + s.name))
        lines.push('')
      }
    ...
    if (matchedAgents.length > 0) {
      ...
      if (g.critical.length) {
        ...
        g.critical.forEach((a) => lines.push('  -> ' + a.name))
        lines.push('')
      }
      if (g.high.length) {
        lines.push('RECOMMENDED AGENTS:')
        g.high.forEach((a) => lines.push('  -> ' + a.name))
        lines.push('')
      }
      if (g.medium.length) {
        lines.push('SUGGESTED AGENTS:')
        g.medium.forEach((a) => lines.push('  -> ' + a.name))
        lines.push('')
      }
      if (g.low.length) {
        lines.push('OPTIONAL AGENTS:')
        g.low.forEach((a) => lines.push('  -> ' + a.name))
        lines.push('')
      }
